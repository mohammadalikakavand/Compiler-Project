...
class Parser1:
    ...
    def create_parse_table(self):
        ...
        file_raw = open('grammer_raw.txt', 'r')
        lines = file.readlines()
    ...
    def parse(self):
        ...
        while True:
            if top[0].startswith('#'):
                stack.pop()
                self.cod_gen(top, token, line_no)
                top = stack[-1][0]
            elif top == 'EPSILON':
                epsilon = stack.pop()
        ...
        for i in range(len(self.symbol_table) - 1, -1, -1):
            if self.symbol_table[i][0] == 'main':
                main = self.symbol_table[i]
                break
        if main is None:
            line_no = len(open("input.txt", "r").read().split("\n"))
            self.semantic_errors.append(f'#{line_no} : Semantic Error! main function not found.')

        if self.first_method != -100 and main is not None:
            self.program_block[self.first_method] = '(JP, ' + str(main[6]) + ', , )'
        ######################
        ...
        for i in range(len(self.program_block)):
            string_file += str(i) + '\t' + self.program_block[i] + '\n'
        print(string_file, file=file)
        file.close()
    ...
    def get_addr(self, lexeme, line_no):
        if lexeme == 'output':
            self.current_called_function = 'output'
            self.func_name = 'output'
        else:
            for element in reversed(self.symbol_table):
                if element[0] == lexeme:
                    if element[1] == 'func':
                        self.current_called_function = element[0]
                        self.func_name = element[0]
                    return element[5]
            if self.assignment_flag == 1 or self.return_flag == 1 or self.rel_flag == 1 or self.number_of_calls > 0:
                self.semantic_errors.append(
                    f"#{line_no} : Semantic Error! '" + lexeme + "' is not defined appropriately.")
                self.func_name = lexeme
                return -1
            self.func_name = lexeme
    ...
    def cod_gen(self, top, token, actual_line_number):
        ...
        # print(a)
        if top == '#pid':
            self.semantic_stack.append(('direct', self.get_addr(token[-1], actual_line_number)))
        elif top == '#push_num':
            ...
            if self.invalid_call == 1:
                self.invalid_call = 0
                self.current_called_function = ''
                self.call_flag = 0
                if self.assignment_flag == 1 or self.return_flag == 1 or self.rel_flag == 1 or self.number_of_calls > 0:
                    self.semantic_stack.append(('literal', '-1'))
            ...
            if self.current_called_function == 'output' or self.semantic_stack[-2][1] == 'output':
                ...
                args_no = self.args_stack.pop()
                print('args', args_no)
                temp = 0
                flag_check = 0
                for i in range(0, len(self.symbol_table), 1):
                ...
                if args_no != temp[2]:
                    self.semantic_errors.append(
                        f"#{actual_line_number} : Semantic Error! Mismatch in numbers of arguments of '{self.current_called_function}'.")
                ...
                self.semantic_stack.pop()
                if temp[7] == 0 and (
                        self.assignment_flag == 1 or self.return_flag == 1 or self.rel_flag == 1 or self.number_of_calls > 0):
                    self.semantic_errors.append(
                        f"#{actual_line_number} : Semantic Error! Void type in operands.")
                    self.semantic_stack.append(('literal', '-1'))
            ...
            self.func_decl[token[-1]] = actual_line_number
            self.current_function = token[-1]
            self.args_stack.append(0)
        elif top == '#add_var':
            for i in range(len(self.symbol_table) - 1, -1, -1):
                if self.symbol_table[i][0] == self.current_function:
                    ...
                    self.semantic_stack.pop()
                    arg_num = self.args_stack.pop()
                    self.args_stack.append(arg_num + 1)
                    break
            ...
            try:
                ...
                self.program_block.pop(-1)
                self.semantic_errors.append(f"#{actual_line_number} : Semantic Error! No 'while' found for 'continue'.")
        elif top == '#break':
            try:
                self.req_for_link.append(('break', len(self.scope_stack), len(self.program_block)))
                self.program_block.append(('none'))
                if self.scope_stack[-1][0] != 'while':
                    self.semantic_errors.append(
                        f"#{actual_line_number} : Semantic Error! No 'while' found for 'break'.")
            except:
                self.semantic_errors.append(f"#{actual_line_number} : Semantic Error! No 'while' found for 'break'.")
        elif top == '#assign_flag':
            self.assignment_flag = 1
        elif top == '#set_call_flag':
            self.number_of_calls += 1
            self.call_flag = 1
            check = -1
            for i in range(len(self.symbol_table)):
                if self.symbol_table[i][0] == self.func_name:
                    check = i
            if self.func_name != 'output' and (check == -1 or check == len(self.symbol_table) - 1):
                self.invalid_call = 1
                if not (
                        self.assignment_flag == 1 or self.return_flag == 1 or self.rel_flag == 1 or self.number_of_calls > 1):
                    self.semantic_errors.append(
                        f"#{actual_line_number} : Semantic Error! '" + self.func_name + "' is not defined appropriately.")
                self.func_name = ''
            else:
                self.args_stack.append(0)

        elif top == '#pop_invalid':
            if self.invalid_call:
                self.semantic_stack.pop()
            else:
                last = self.args_stack.pop()
                self.args_stack.append(last + 1)
        elif top == '#check_for_mismatch':
            arg_no = self.args_stack.pop()
            for element1 in self.symbol_table:
                for element2 in self.symbol_table:
                    if element1[0] == element2[0] == self.current_function and element1[2] == element2[
                        2] == arg_no and element1 != element2:
                        self.semantic_errors.append(
                            f"#{actual_line_number} : Semantic Error! Function '" + element1[
                                0] + "' has already been defined with this number of arguments.")
                        return
        elif top == '#set_ret_flag':
            self.return_flag = 1
        elif top == '#rel_start':
            self.rel_flag = 1
        else:
